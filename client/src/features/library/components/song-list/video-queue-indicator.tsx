...
import { useVideoQueue } from '@/features/library/queries/use-songs';

function label(youtubeCount: number, reelCount: number): string {
  const parts: string[] = [];
  if (youtubeCount > 0) {
    parts.push(`${youtubeCount} YouTube video${youtubeCount === 1 ? '' : 's'}`);
  }
  if (reelCount > 0) {
    parts.push(`${reelCount} karaoke render${reelCount === 1 ? '' : 's'}`);
  }
