...
import type { TimeSubscriber } from '@/features/playback/hooks/use-audio-player';
...
type UseSourceVideoSyncOptions = {
...
export function useSourceVideoSync({
  ...
  useEffect(() => {
    ...
    if (!video || typeof src !== 'string' || src === '') {
      return undefined;
    }
  ...
  }, [currentRatio, getCurrentTime, toSourceTime, videoRef]);
